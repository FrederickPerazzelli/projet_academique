...
But du programme:	Méthodes et prototypes de l'objet game
                    cet objet crée une boucle qui permet a 
...
class game {
    ...
    public:
        ...
        point randPosition() const; //génère une nouvelle position aléatoire 
                                    //ds le terrain

        void createApple();         //génère une nouvelle pomme tq position 
                                    //est ds le snake
        ...
        bool canMove(const point& p) const; //retourne vrai si la tête du 
                                            //snake peut bouger

        int getScore() const;       //retourne le score (seule info qui 
                                    //pourrait être intéressante
                                    //de l’extérieur) et aucun setteur 
                                    //pour cet objet.
        ...
        void drawScreen(ostream& output);           // dessine la game a l'écran
};
